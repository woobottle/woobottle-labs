  ...
  const handleShare = async () => {
    ...
    try {
      ...
      if (supportsFileShare && (navigator as any).canShare({ files: [file] })) {
        ...
        alert(
          "이 브라우저는 공유 기능을 지원하지 않습니다. 다운로드를 사용해주세요.",
        );
      }
    } catch (e) {
    ...
    <div className="max-w-3xl mx-auto">
      <div className="grid gap-6 md:grid-cols-[1fr_auto] items-start">
        ...
        <div className="flex flex-col items-center gap-3">
          {/* QR 코드 자체는 스캔/인쇄 가독성을 위해 흰 배경 유지 */}
          <div
            ref={containerRef}
            className="p-4 bg-white rounded-2xl border border-[#1A1A1A]"
            style={{ width: size, height: size }}
          />
          <p className="text-xs text-[#525252]">사이즈: {size}px</p>
        </div>
      </div>
    </div>
  );
