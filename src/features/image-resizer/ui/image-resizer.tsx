...
export const ImageResizer: React.FC = () => {
  ...
  return (
    <div className="max-w-6xl mx-auto p-6 space-y-6">
      {/* 설정 패널 */}
      <Card className="p-6 bg-[#0A0A0A] border border-[#1A1A1A]">
        <div className="flex items-center gap-2 mb-4">
          <Settings className="w-5 h-5 text-[#A3A3A3]" strokeWidth={1.5} />
          <h2 className="text-lg font-semibold text-white">리사이징 설정</h2>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div>
            ...
            <Input
              ...
              onChange={(e) =>
                handleOptionChange("quality", Number(e.target.value))
              }
              className="w-full"
            />
          </div>
          ...
          <div>
            ...
            <select
              ...
              className="w-full px-3 py-2 border border-[#1A1A1A] rounded-lg bg-[#0A0A0A] text-white focus:ring-2 focus:ring-white/20 focus:border-transparent"
            >
        ...
        <div className="mt-4">
          <label className="flex items-center gap-2">
            <input
              ...
              onChange={(e) =>
                handleOptionChange("maintainAspectRatio", e.target.checked)
              }
              className="rounded border-[#1A1A1A] text-white focus:ring-white/20"
            />
            <span className="text-sm text-[#A3A3A3]">종횡비 유지</span>
          </label>
        </div>
      </Card>
      ...
      <Card className="p-6 bg-[#0A0A0A] border border-[#1A1A1A]">
        <div
          className="border-2 border-dashed border-[#2A2A2A] rounded-lg p-8 text-center hover:border-white hover:bg-[#0A0A0A] transition-colors duration-150 cursor-pointer"
          onDrop={handleDrop}
          onDragOver={handleDragOver}
          onClick={() => fileInputRef.current?.click()}
        >
          <Upload
            className="w-12 h-12 text-[#525252] mx-auto mb-4"
            strokeWidth={1.5}
          />
          <p className="text-lg font-medium text-[#A3A3A3] mb-2">
        ...
        {state.isLoading && (
          <div className="mt-4 text-center">
            ...
            <div className="w-full bg-[#1A1A1A] rounded-full h-2">
              <div
                className="bg-white h-2 rounded-full transition-colors duration-150"
        ...
        {state.error && (
          <div className="mt-4 p-4 bg-[#0A0A0A] border border-[#1A1A1A] rounded-lg">
            <p className="text-[#A3A3A3]">{state.error}</p>
          </div>
        )}
      </Card>
        ...
        <Card className="p-6 bg-[#0A0A0A] border border-[#1A1A1A]">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex gap-2">
              ...
              <Button onClick={handleResize}>리사이징 시작</Button>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {uploadedImages.map((image, index) => {
              ...
              if (
                ...
                const widthZoom =
                  displayWidth < minDisplaySize
                    ? minDisplaySize / displayWidth
                    : 1;
                const heightZoom =
                  displayHeight < minDisplaySize
                    ? minDisplaySize / displayHeight
                    : 1;
                ...
                >
                  <div className="space-y-3">
                    ...
                    <div>
                      ...
                      <div className="relative bg-[#0A0A0A] rounded border border-[#1A1A1A] overflow-hidden">
                        <img
                        ...
                        />
                        {previewUrls[index] && (
                          <div className="absolute top-2 left-2 bg-black/80 text-white text-xs px-2 py-1 rounded border border-[#1A1A1A]">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="relative">
                      <img
                        ...
                        alt="원본"
                        className="w-full h-32 object-cover rounded border border-[#1A1A1A]"
                      />
                      <div className="absolute bottom-0 left-0 bg-black/80 text-white text-xs px-2 py-1 rounded border border-[#1A1A1A]">
                        {result.original.dimensions?.width}×
                        {result.original.dimensions?.height}
                      </div>
                    </div>
      ...
      <Card className="p-6 bg-[#0A0A0A] border border-[#1A1A1A]">
        <div className="flex items-start gap-3">
          <ImageIcon
            className="w-5 h-5 text-[#A3A3A3] mt-0.5"
            strokeWidth={1.5}
          />
          <div>
            <h3 className="font-medium text-white mb-2">사용법</h3>
            <ul className="text-sm text-[#A3A3A3] space-y-1">
              <li>
                • 이미지를 업로드하면 실시간으로 설정에 맞는 미리보기가
                표시됩니다
              </li>
              <li>
                • 가로/세로 크기를 입력하거나 둘 중 하나만 입력하여 자동 계산
              </li>
              <li>• 종횡비 유지를 해제하면 지정한 크기로 강제 조절</li>
              <li>• 품질은 0.1~1.0 사이 값 (1.0이 최고 품질)</li>
              <li>
                • 미리보기가 마음에 들면 &quot;리사이징 시작&quot; 버튼을 눌러
                최종 파일 생성
              </li>
