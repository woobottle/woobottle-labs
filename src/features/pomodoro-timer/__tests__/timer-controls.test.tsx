...
describe("TimerControls", () => {
  ...
  describe("버튼 스타일", () => {
    it("should apply correct variant classes to buttons", () => {
      ...
      expect(startButton).toHaveClass("rounded-lg", "font-medium");
      expect(resetButton).toHaveClass("rounded-lg", "font-medium");
      expect(skipButton).toHaveClass("rounded-lg", "font-medium");
    });
    ...
    it("should apply warning variant to pause button", () => {
      ...
      expect(pauseButton).toHaveClass(
        "border",
        "border-[#2A2A2A]",
        "text-white",
      );
